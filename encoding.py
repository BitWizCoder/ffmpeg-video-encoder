...
def convert_video(input_file, output_file, output_resolution):
    ...
    for line in process.stderr:
        ...
        if "frame=" in line and ("fps=" in line or "fps =" in line) and "time=" in line:
            frame_info_start = line.find("frame=") + len("frame=")
            frame_info_end = line.find("fps=")
            if frame_info_start < frame_info_end:
                frame_info = line[frame_info_start:frame_info_end].strip()
...
video_files = [file for file in os.listdir() if file.lower().endswith(".mp4")]
...
for input_file in video_files:
    output_file = os.path.join(output_directory, os.path.splitext(input_file)[0] + f".mp4")
    total_frames = int(subprocess.check_output(["ffprobe", "-v", "error", "-select_streams", "v:0", "-show_entries", "stream=nb_frames", "-of", "default=nokey=1:noprint_wrappers=1", input_file], universal_newlines=True))
    convert_video(input_file, output_file, output_resolution)
